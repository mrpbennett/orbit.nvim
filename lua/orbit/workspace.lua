...
-- fixed filter line of the sidebar buffer) and strip the "Filter: " prefix so
-- callers get just the raw filter text.
--   state: the workspace state table.
-- Returns: the filter string (may be "").
local function filter_text(state)
	local line = vim.api.nvim_buf_get_lines(state.sidebar, FILTER_LINE - 1, FILTER_LINE, false)[1] or "Filter: "
...
local function load_metadata(state, profile, row, category, show_progress)
	...
	if
		schema_tree.is_metadata_loaded(state.tree, row, category.id)
		or schema_tree.is_metadata_loading(state.tree, row, category.id)
	then
		return
	end
...
-- makes the sidebar buffer modifiable; moves the cursor to the fixed
...
local function focus_filter(state)
	...
	vim.bo[state.sidebar].modifiable = true
	vim.api.nvim_win_set_cursor(state.sidebar_window, { FILTER_LINE, #"Filter: " + #state.filter })
...
local function configure_sidebar(state)
	...
	vim.api.nvim_buf_attach(state.sidebar, false, {
		on_lines = function()
			-- Only edits made while the filter is modifiable schedule a redraw.
	...
	vim.keymap.set(
		...
		"h",
		collapse_current,
		{ buffer = state.sidebar, silent = true, nowait = true, desc = "Collapse Orbit node" }
	)
	vim.keymap.set(
		"n",
		"l",
		expand_current,
		{ buffer = state.sidebar, silent = true, nowait = true, desc = "Expand Orbit node" }
	)
	vim.keymap.set(
		"n",
		"<CR>",
		activate_current,
		{ buffer = state.sidebar, silent = true, nowait = true, desc = "Bind Orbit profile" }
	)
