...
-- Builds an "AND table_schema IN (...)" SQL fragment to restrict schema
-- listing queries to a configured allow-list of schemas (the
-- `schema_patterns` profile option). Each schema name is turned into a
-- proper SQL string literal via `literal()` to avoid injection issues.
-- Params: schemas - a Lua array of schema name strings, or nil.
...
-- grid, and if so, what information is needed to write changes back to the
-- database. Editing requires knowing the row's unique identity, which is
-- only possible when the row came from an actual table (not a view or an
-- arbitrary SELECT) and that table has at least one primary key column.
-- Params:
--   _ - options (unused).
--   row - metadata about the source of the result set (row.type is "table",
--     "view", etc, plus .name/.schema).
--   primary_keys - a Lua array of primary key column names already
--     discovered for this table (empty if none, or if not a table).
-- Returns: on success, a table {name, schema, primary_keys} describing the
--   editable target; on failure, `nil, error_message` explaining why the
...
-- statements once the user has made changes and asked to save them.
-- Params:
--   _ - options (unused).
--   target - the editable-table descriptor from M.editable_table: has
--     .name, .schema, and .primary_keys (the column(s) that uniquely
--     identify a row).
...
-- Returns: on success, one big SQL string containing "BEGIN; ...; COMMIT;"
--   wrapping all the generated statements, so they run as a single
--   transaction (all succeed or all fail together). On failure (e.g. a row
--   can't be uniquely identified), returns `nil, error_message` and no SQL
--   is generated.
-- Side effects: none (pure string-building); it's the caller's job to
--   actually execute the returned SQL.
function M.mutation_statement(_, target, changes)
  local name = qualified(target)
  local primary_keys = target.primary_keys
  -- Renders a single cell's value as SQL: vim.NIL is Neovim's sentinel for a
  -- real SQL NULL (as opposed to Lua's `nil`, which just means "no key"), so
  -- it must map to the literal keyword NULL rather than being quoted like a
  -- string.
  ...
  -- using the row's *original* (pre-edit) primary key values, since that's
  -- what still identifies the row in the database.
  ...
  for _, row in ipairs(changes.inserted) do
    ...
    table.sort(columns)
    if #columns == 0 then
      -- Every column was left unset: fall back to Postgres's
      -- "INSERT INTO x DEFAULT VALUES" form, since "INSERT INTO x () VALUES ()"
      -- isn't valid syntax.
