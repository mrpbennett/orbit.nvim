...
function M.validate_options(profile_name, options)
	...
	if
		options.port ~= nil
		and (type(options.port) ~= "number" or options.port % 1 ~= 0 or options.port < 1 or options.port > 65535)
	then
...
function M.prepare(options, statement)
	...
	append(command, {
		...
		"--pset",
		"footer=off", -- suppress the "(N rows)" footer line psql normally prints
		"--set",
		"ON_ERROR_STOP=on", -- abort immediately on the first SQL error (one-shot run)
		"--command",
		statement,
	})
	return command
