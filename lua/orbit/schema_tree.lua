	...
	for _, schema_group in ipairs(groups) do
		...
		for _, kind in ipairs({ "tables", "views" }) do
			...
			if schema_expanded and #objects > 0 then
				...
				local marker = icons.collapsed
				if group_expanded then
					marker = icons.expanded
				end
				local label = string.format("%s %d", kind, #objects)
				if kind == "tables" then
							...
							for _, category in
								ipairs(
									connector
											...
											and connector.metadata_categories(profile.options, row)
										or {}
								)
							do
								local metadata_node =
									{ category = category, kind = "metadata", profile = profile, row = row }
								...
								local label = entries and string.format("%s %d", category.label, #entries)
									or category.label
								table.insert(
									lines,
									string.format(
