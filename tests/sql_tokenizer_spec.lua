...
return {
	...
	["SQL Server mode tokenizes escaped bracket identifiers as one name"] = function()
	...
	["SQL Server batch separator detection ignores strings comments and ordinary names"] = function()
		assert(tokenizer.has_sqlserver_batch_separator({ "SELECT 1", "GO -- next batch" }))
		assert(tokenizer.has_sqlserver_batch_separator({ "GO 2;" }))
		assert(tokenizer.has_sqlserver_batch_separator({ "GO nonsense" }))
		assert(tokenizer.has_sqlserver_batch_separator({ "GO /* comment */" }))
		assert(not tokenizer.has_sqlserver_batch_separator({ "GO2" }))
		assert(not tokenizer.has_sqlserver_batch_separator({ "GO_PROC" }))
		assert(not tokenizer.has_sqlserver_batch_separator({ "SELECT 'GO', [GO], go FROM dbo.words -- GO" }))
		assert(not tokenizer.has_sqlserver_batch_separator({ "SELECT 'open", "GO", "close'" }))
		assert(not tokenizer.has_sqlserver_batch_separator({ "/* open", "GO", "close */ SELECT 1" }))
	end,
	...
	["SQL Server mode does not treat PostgreSQL dollar quotes as strings"] = function()
