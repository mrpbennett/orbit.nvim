...
- When a user corrects a partial namespace migration, extend the rename to filenames, imports, helper classes, and documentation paths rather than preserving internal legacy names by default.
...
- For Redis JSON presentation, distinguish the `redis-cli --json` transport envelope from a bulk string whose contents are themselves a serialized JSON object or array.
